...
export const registrationCheckoutPayloadSchema = z
  .object({
    ...
    adultGuestCount: z.coerce.number().int().min(0).max(20),
    childGuestCount: z.coerce.number().int().min(0).max(20),
...
function getRegistrationRsvpPartyCounts(payload: RegistrationCheckoutPayload) {
  const golferIsAdult = payload.age >= 18;

  return {
    adultAttendeeCount: payload.adultGuestCount + (golferIsAdult ? 1 : 0),
    childAttendeeCount: payload.childGuestCount + (golferIsAdult ? 0 : 1),
  };
}
...
async function syncRegistrationRsvp(options: {
  ...
  if (!options.payload.dayBeforeRsvp) {
    return;
  }
  ...
  try {
    return await db.$transaction(
      async (transaction) => {
        ...
        const registration = await transaction.registration.upsert({
          where: { paymentReference: checkout.paymentReference },
          update: {
          ...
          create: {
            ...
            adultGuestCount: payload.adultGuestCount,
            childGuestCount: payload.childGuestCount,
            dayBeforeRsvp: payload.dayBeforeRsvp,
            paymentStatus: "CONFIRMED",
            paymentReference: checkout.paymentReference,
            notes: payload.notes,
          },

...
const unpublishConflictMessage =
...
// Only same-app relative paths under /chair/pairings are honored. Absolute
// URLs, protocol-relative paths, backslashes, and other routes fall back to
// the bare pairings page so the returned navigation target can never leave
// the pairings area.
function resolvePairingsReturnUrl(formData: FormData | undefined) {
  ...
  if (
    ...
    if (
      url.pathname === chairPairingsPath ||
      url.pathname.startsWith(`${chairPairingsPath}/`)
    ) {
      return url;
    }
  }

  return new URL(chairPairingsPath, internalUrlBase);
}

function pairingsOutcomeResult(
  formData: FormData | undefined,
  outcome: PairingNoticeCode,
): ChairActionResult {
  const url = resolvePairingsReturnUrl(formData);
  url.searchParams.set(PAIRINGS_NOTICE_PARAM, outcome);
...
async function runPairingAction(
  formData: FormData | undefined,
  successOutcome: PairingNoticeCode,
  operation: () => Promise<void>,
): Promise<ChairActionResult> {
  let outcome: PairingNoticeCode = "action-failed";

  try {
    await operation();
    outcome = successOutcome;
  } catch (error) {
    console.error("chair pairing action failed", error);
    outcome = "action-failed";
  }

  return pairingsOutcomeResult(formData, outcome);
}
...
export async function generatePairings(formData?: FormData) {
  ...
  return runPairingAction(formData, "generated", async () => {
...
export async function createPairingGroup(formData: FormData) {
  ...
  return runPairingAction(formData, "group-created", async () => {
...
export async function deletePairingGroup(formData: FormData) {
  ...
  return runPairingAction(formData, "group-deleted", async () => {
...
export async function updatePairingGroup(formData: FormData) {
  ...
  return runPairingAction(formData, "group-updated", async () => {
...
export async function assignPairingMember(formData: FormData) {
  ...
  return runPairingAction(formData, "member-assigned", async () => {
    ...
    await db.$transaction(async (transaction) => {
      ...
      if (membershipsToMove.length) {
        ...
        const sourceGroupIds = [
          ...new Set(
            membershipsToMove.map((membership) => membership.groupId),
          ),
        ];
...
export async function removePairingMember(formData: FormData) {
  ...
  return runPairingAction(formData, "member-removed", async () => {
...
export async function unpublishPairings(formData?: FormData) {
  await requireChairSession();

  let outcome: PairingNoticeCode = "unpublish-error";
  ...
  try {
    await db.$transaction(async (transaction) => {
      ...
      if (draftGroups.length > 0) {
        throw new Error(unpublishConflictMessage);
      }
    ...
    const isConflict =
      error instanceof Error && error.message === unpublishConflictMessage;

    if (!isConflict) {
      console.error("chair pairing unpublish failed", error);
    }

    outcome = isConflict ? "unpublish-conflict" : "unpublish-error";
  }
...
export async function publishPairings(formData?: FormData) {
  ...
  return runPairingAction(formData, "published", async () => {
    await db.$transaction(async (transaction) => {

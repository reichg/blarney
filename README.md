...
- Shared pagination is URL-driven on the home page published pairings section, public `/photos`, and chair list pages `/chair/registrations`, `/chair/rsvps`, `/chair/feedback`, `/chair/pairings`, `/chair/remembrance`, and `/chair/photos`. The default page size is 50, the maximum page size is 50, and the UI does not expose a page-size picker.
...
- Remembrance submissions store as private feedback with category `In Remembrance`, so the chair reviews them on `/chair/feedback` alongside other private feedback. Name and email are optional for note-only submissions and required only when photos are included.
...
- Verify remembrance note submission: open `/remembrance` from the main nav, submit a note without contact info, and confirm it appears only on `/chair/feedback` with category `In Remembrance`.
...
- Verify list pagination: on the home page published pairings section, public `/photos`, and chair list pages (`/chair/registrations`, `/chair/rsvps`, `/chair/feedback`, `/chair/pairings`, `/chair/remembrance`), use next/previous navigation and refresh to confirm the current page stays URL-driven. Confirm each screen stays fixed at 50 items per page and the UI does not expose a page-size picker.

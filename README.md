...
- Shared pagination is URL-driven on the home page published pairings section, public `/photos`, and chair list pages `/chair/registrations`, `/chair/rsvps`, `/chair/feedback`, `/chair/remembrance`, and `/chair/photos`. The default page size is 50, the maximum page size is 50, and the UI does not expose a page-size picker. `/chair/pairings` uses an unassigned-golfers roster plus draft-group previews instead of paginating the pairing workspace.

...
- Shared pagination is URL-driven on the home page published pairings section, public `/photos`, and chair list pages `/chair/registrations`, `/chair/rsvps`, `/chair/feedback`, `/chair/remembrance`, and `/chair/photos`. The default page size is 50, the maximum page size is 50, and the UI does not expose a page-size picker. `/chair/pairings` uses an all-golfers workspace instead of paginating the roster.
...
- Public `/photos` shows approved gallery photos only, approved gallery images can be opened full size from that page, and the list uses the shared URL-driven pager. Pending and rejected submissions must stay private to chair routes, and remembrance uploads never appear in the public gallery.
...
- Chair review at `/chair/photos` remains gallery-only. Gallery uploads start in the private S3 `pending/` prefix, can be approved or rejected by the chair, and only approved gallery submissions with an `approved/` key appear on `/photos`. The pending and reviewed lists paginate independently, so moving one list preserves the other list's current page. Returning an approved gallery photo to pending moves the object back to its original `pending/` key and immediately removes it from the public view.
...
- Permanent deletion is limited to pending photos. That action deletes the pending S3 object and removes the `PhotoSubmission` row so the photo no longer appears in chair review.
...
- **All-golfers roster**: The pairings page displays all paid golfers in one roster so the chair can see who is already assigned to a DRAFT group and who is still unassigned. The Assign and Move controls place each golfer in the selected draft group; the Remove control returns a golfer to the unassigned state.
- **Draft group editing**: The chair can create empty custom draft groups, assign golfers, move golfers between draft groups, remove golfers from draft groups, edit group name, display order, and optional tee time, and delete empty draft groups. Groups with members must be emptied before deletion.
...
- Verify all-golfers roster and assignment: on `/chair/pairings`, confirm the roster displays all paid golfers and clearly shows which golfers are assigned to DRAFT groups and which are unassigned. Use Assign for an unassigned golfer and verify they join the selected draft group; use Remove on a group member and verify they return to the unassigned state.
- Verify draft group editing: create a custom empty draft group, assign golfers, move golfers between draft groups, edit group name/display order/optional tee time, remove golfers from draft groups, and delete empty draft groups. Confirm groups with members cannot be deleted until emptied.
